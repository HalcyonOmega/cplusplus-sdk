...
public:
	...
	[[nodiscard]] static ValidationResult ValidateAgainstSchema(const JSONData& InData, const JSONSchema& InSchema);

	// Type-specific validation methods
	[[nodiscard]] static ValidationResult ValidateObjectType(const JSONData& InData, const JSONSchema& InSchema);
	[[nodiscard]] static ValidationResult ValidateArrayType(const JSONData& InData, const JSONSchema& InSchema);
	[[nodiscard]] static ValidationResult ValidateStringType(const JSONData& InData, const JSONSchema& InSchema);
	[[nodiscard]] static ValidationResult ValidateNumberType(const JSONData& InData, const JSONSchema& InSchema);
	[[nodiscard]] static ValidationResult ValidateBooleanType(const JSONData& InData, const JSONSchema& InSchema);
	...
	[[nodiscard]] static ValidationResult ValidateAllOf(const JSONData& InData,
		const std::vector<JSONSchema>& InSchemas);
	[[nodiscard]] static ValidationResult ValidateNot(const JSONData& InData, const JSONSchema& InSchema);
	[[nodiscard]] static ValidationResult ValidateConditional(const JSONData& InData,
...
private:
	...
	ValidateRecursive(const JSONData& InData, const JSONSchema& InSchema, std::string_view InPath = "");

	// Advanced constraint validation
	[[nodiscard]] static ValidationResult
	ValidateStringConstraints(const JSONData& InData, const JSONSchema& InSchema, std::string_view InPath);
	[[nodiscard]] static ValidationResult
	ValidateNumberConstraints(const JSONData& InData, const JSONSchema& InSchema, std::string_view InPath);
	[[nodiscard]] static ValidationResult
	ValidateArrayConstraints(const JSONData& InData, const JSONSchema& InSchema, std::string_view InPath);
	[[nodiscard]] static ValidationResult
	ValidateObjectConstraints(const JSONData& InData, const JSONSchema& InSchema, std::string_view InPath);
};
