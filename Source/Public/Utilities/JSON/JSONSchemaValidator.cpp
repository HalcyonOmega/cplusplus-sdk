...
JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateAgainstSchema(const JSONData& InData,
	const JSONSchema& InSchema)
{
	return ValidateRecursive(InData, InSchema, "");
}

JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateRecursive(const JSONData& InData,
	const JSONSchema& InSchema,
	const std::string_view InPath)
{
	...
	if (!IsValidType(InData, InSchema.Type))
	{
		...
		result.AddError(
			"Type mismatch at " + path + ": expected '" + InSchema.Type + "', got '" + GetJSONType(InData) + "'");
		return result;
	}
	...
	{
		if (auto [IsValid, Errors] = ValidateObjectType(InData, InSchema); !IsValid)
		{
	...
	{
		if (auto [IsValid, Errors] = ValidateArrayType(InData, InSchema); !IsValid)
		{
			for (const auto& error : Errors)
			{
				result.AddError(InPath.empty() ? error : std::string(InPath) + "." + error);
	...
	{
		if (auto [IsValid, Errors] = ValidateStringType(InData, InSchema); !IsValid)
		{
			for (const auto& error : Errors)
			{
				result.AddError(InPath.empty() ? error : std::string(InPath) + "." + error);
	...
	{
		if (auto [IsValid, Errors] = ValidateNumberType(InData, InSchema); !IsValid)
		{
			for (const auto& error : Errors)
			{
				result.AddError(InPath.empty() ? error : std::string(InPath) + "." + error);
	...
	{
		if (auto [IsValid, Errors] = ValidateBooleanType(InData, InSchema); !IsValid)
		{
			for (const auto& error : Errors)
			{
				result.AddError(InPath.empty() ? error : std::string(InPath) + "." + error);
...
JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateObjectType(const JSONData& InData,
	const JSONSchema& InSchema)
{
	ValidationResult result;
		...
		for (const auto& [key, value] : InData.items())
		{
			if (InSchema.Properties->contains(key))
			{
				...
				if (const auto& propSchema = InSchema.Properties->at(key); propSchema.contains("type"))
				{
					...
					{
						result.AddError("Property '" + key + "': expected type '" + expectedType + "', got '"
							+ GetJSONType(value) + "'");
					}
				}
			}
...
JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateArrayType(const JSONData& InData,
	const JSONSchema& /*InSchema*/)
{
	ValidationResult result;
...
JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateStringType(const JSONData& InData,
	const JSONSchema& /*InSchema*/)
{
	ValidationResult result;
...
JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateNumberType(const JSONData& InData,
	const JSONSchema& InSchema)
{
	ValidationResult result;
...
JSONSchemaValidator::ValidationResult JSONSchemaValidator::ValidateBooleanType(const JSONData& InData,
	const JSONSchema& /*InSchema*/)
{
	ValidationResult result;

	if (!InData.is_boolean())
	{
		result.AddError("Expected boolean type");
	}

	return result;
}

bool JSONSchemaValidator::IsValidType(const JSONData& InData, const std::string_view InType)
{
	if (InType == "object")
	{
		return InData.is_object();
	}
	if (InType == "array")
	{
		return InData.is_array();
	}
	if (InType == "string")
	{
		return InData.is_string();
	}
	if (InType == "number")
	{
		return InData.is_number();
	}
	if (InType == "integer")
	{
		return InData.is_number_integer();
	}
	if (InType == "boolean")
	{
		return InData.is_boolean();
	}
	if (InType == "null")
	{
		return InData.is_null();
	}
