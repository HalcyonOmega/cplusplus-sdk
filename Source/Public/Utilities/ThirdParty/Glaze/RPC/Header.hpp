...
inline action pack_action(bool notify, bool read, bool write) noexcept {
    return static_cast<action>((notify ? uint32_t(action::notify) : 0)
...
struct header {
    ...
    uint64_t query_length{
        no_length_provided}; // The total length of the query (-1 denotes no size given)
    //
    uint64_t body_length{
        no_length_provided}; // The total length of the body (-1 denotes no size given)
    //
