...
struct streaming_connection : public std::enable_shared_from_this<streaming_connection> {
    ...
    void send_headers(int status_code,
    ...
    void send_chunk(std::string_view data, data_sent_handler handler = {}) {
        ...
        if (chunked_encoding_) {
            ...
            if (auto [ptr, ec] = std::to_chars(size_buf.data(), size_buf.data() + size_buf.size(),
                                               data.size(), 16);
                ec == std::errc()) {
    ...
    template <class T>
    void send_json_event(const T& data, std::string_view event_type = "message",
                         std::string_view id = {}, data_sent_handler handler = {}) {
        std::string json_str;
...
struct streaming_response {
    ...
    streaming_response&
    start_stream(int status_code = 200,
                 const std::unordered_map<std::string, std::string>& headers = {}) {
        if (stream) { stream->send_headers(status_code, headers); }
        return *this;
    }

    // Send a chunk of data
    streaming_response& send(std::string_view data) {
        if (stream) { stream->send_chunk(data); }
        return *this;
    }
    ...
    template <class T> streaming_response& send_json(const T& data) {
        if (stream) {
            std::string json_str;
            auto ec = glz::write_json(data, json_str);
            if (!ec) { stream->send_chunk(json_str); }
        }
        return *this;
    }

    // Send Server-Sent Event
    streaming_response& send_event(std::string_view event_type, std::string_view data,
                                   std::string_view id = {}) {
        if (stream) { stream->send_event(event_type, data, id); }
        return *this;
    }
...
template <bool EnableTLS = false> struct http_server {
    ...
    inline http_server() : io_context(std::make_unique<asio::io_context>()) {
        error_handler = [](std::error_code ec, std::source_location loc) {
            std::fprintf(stderr, "Error at %s:%d: %s\n", loc.file_name(),
                         static_cast<int>(loc.line()), ec.message().c_str());
        };
        ...
        if constexpr (EnableTLS) {
            ...
            static_assert(
                !EnableTLS,
                "TLS support requires GLZ_ENABLE_SSL to be defined and OpenSSL to be available");
#endif
        }
    }
    ...
    inline http_server& bind(std::string_view address, uint16_t port) {
        try {
            ...
            error_handler(std::make_error_code(std::errc::address_in_use),
        ...
        for (size_t i = 0; i < num_threads; ++i) {
            threads.emplace_back([this] {
                try {
                    ...
                    error_handler(std::make_error_code(std::errc::operation_canceled),
                                  std::source_location::current());
                }
            });
        }
    ...
    inline http_server& mount(std::string_view base_path, const http_router& router) {
        ...
        // Add middleware
        for (const auto& middleware : router.middlewares) { root_router.use(middleware); }
    ...
    inline http_router& get(std::string_view path, handler handle) {
        return root_router.get(path, handle);
    }

    inline http_router& post(std::string_view path, handler handle) {
        return root_router.post(path, handle);
    }

    inline http_router& put(std::string_view path, handler handle) {
        return root_router.put(path, handle);
    }

    inline http_router& del(std::string_view path, handler handle) {
        return root_router.del(path, handle);
    }

    inline http_router& patch(std::string_view path, handler handle) {
        return root_router.patch(path, handle);
    }

    // Register streaming route
    inline http_server& stream(http_method method, std::string_view path,
                               streaming_handler handle) {
     ...
     *
     * @param origins Vector of allowed origins (e.g., {"https://example.com",
     * "https://app.example.com"})
     * @param allow_credentials Whether to allow credentials (cookies, auth headers)
     * @return Reference to this server for method chaining
     */
    http_server& enable_cors(const std::vector<std::string>& origins,
     ...
     * @return Reference to this server for method chaining
     */
    inline http_server& load_certificate(const std::string& cert_file,
  ...
  private:
    ...
    std::unordered_map<std::string, std::unordered_map<http_method, streaming_handler>>
        streaming_handlers_;
    ...
    inline void process_request(asio::ip::tcp::socket socket) {
        ...
        asio::async_read_until(
            *socket_ptr, *buffer, "\r\n\r\n",
            [this, socket_ptr, buffer, remote_endpoint](std::error_code ec,
                ...
                if (method_sv.empty()
                    || !std::all_of(method_sv.begin(), method_sv.end(),
                ...
                if (dot_pos == std::string_view::npos || dot_pos == 0
                    || dot_pos == version_number.length() - 1) {
                    send_error_response(socket_ptr, 400, "Bad Request");
                    return;
                }
                std::string_view major_v = version_number.substr(0, dot_pos);
                std::string_view minor_v = version_number.substr(dot_pos + 1);
                if (major_v.empty() || !std::all_of(major_v.begin(), major_v.end(), ::isdigit)
                    || minor_v.empty() || !std::all_of(minor_v.begin(), minor_v.end(), ::isdigit)) {
                    send_error_response(socket_ptr, 400, "Bad Request");
                    return;
                }
                ...
                if (is_websocket_upgrade(headers)) {
                    handle_websocket_upgrade(socket_ptr, *method_opt, target, headers,
                                             remote_endpoint);
                    return;
                }
                ...
                if (content_length > 0) {
                    ...
                    if (body.length() < content_length) {
                        asio::async_read(
                            *socket_ptr, *buffer,
                            asio::transfer_exactly(content_length - body.length()),
                            [this, socket_ptr, buffer, method_opt, target, headers, remote_endpoint,
                             body = std::move(body)](std::error_code ec, size_t) mutable {
                                ...
                                body.append(asio::buffer_cast<const char*>(buffer->data()),
                                            buffer->size());
    ...
    inline void
    handle_websocket_upgrade(std::shared_ptr<asio::ip::tcp::socket> socket, http_method method,
        ...
        auto ws_conn =
            std::make_shared<websocket_connection>(std::move(*socket), ws_it->second.get());
        ws_conn->start(req);
    }

    inline void
    handle_streaming_request(std::shared_ptr<asio::ip::tcp::socket> socket, http_method method,
                             const std::string& target,
                             const std::unordered_map<std::string, std::string>& headers,
    ...
    inline void process_full_request(std::shared_ptr<asio::ip::tcp::socket> socket,
                                     http_method method, const std::string& target,
                                     const std::unordered_map<std::string, std::string>& headers,
                                     std::string body, asio::ip::tcp::endpoint remote_endpoint) {
        ...
        if (handler_it != streaming_handlers_.end()) {
            ...
            if (method_it != handler_it->second.end()) {
                ...
                handle_streaming_request(socket, method, target, headers, std::move(body),
                                         remote_endpoint, method_it->second);
                return;
            }
        }
        ...
        // Find a matching route using http_router::match which handles both exact and parameterized
        // routes
    ...
    inline void send_response(std::shared_ptr<asio::ip::tcp::socket> socket,
        ...
        asio::async_write(
            *socket, asio::buffer(*response_buffer),
...
template <typename T>
void send_periodic_data(std::shared_ptr<streaming_connection> conn,
                        std::function<T()> data_generator, std::chrono::milliseconds interval,
                        size_t max_events = 0) {
    ...
    std::function<void()> send_next = [=]() mutable {
        ...
        } catch (const std::exception&) { conn->close(); }
    };
...
void stream_collection(
    std::shared_ptr<streaming_connection> conn, const Container& data,
