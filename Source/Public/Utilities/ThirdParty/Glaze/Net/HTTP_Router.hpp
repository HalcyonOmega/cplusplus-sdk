...
struct response {
    ...
    template <auto Opts, class T> response& body(T&& value) {
        ...
        if (ec) {
            ...
            response_body =
                R"({"error":"glz::write_json error"})"; // rare that this would ever happen
        }
        return *this;
    }
...
struct http_router {
    ...
    struct RadixNode {
         ...
         */
        std::unordered_map<http_method, std::unordered_map<std::string, param_constraint>>
            constraints;
            ...
            switch (state) {
                ...
                case State::CharClass:
                    if (pattern[p_pos] == ']') {
                        ...
                        if (pattern[p_pos] == value[v_pos]) { char_class_match = true; }
                        p_pos++;
                    }
                    break;
            }
     ...
     */
    inline http_router&
    route(http_method method, std::string_view path, handler handle,
     ...
     */
    inline http_router&
    get(std::string_view path, handler handle,
        const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    post(std::string_view path, handler handle,
         const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    put(std::string_view path, handler handle,
        const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    del(std::string_view path, handler handle,
        const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    patch(std::string_view path, handler handle,
          const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    route_async(http_method method, std::string_view path, async_handler handle,
                const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    get_async(std::string_view path, async_handler handle,
              const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    post_async(std::string_view path, async_handler handle,
               const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    put_async(std::string_view path, async_handler handle,
              const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    del_async(std::string_view path, async_handler handle,
              const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline http_router&
    patch_async(std::string_view path, async_handler handle,
                const std::unordered_map<std::string, param_constraint>& constraints = {}) {
     ...
     */
    inline std::pair<handler, std::unordered_map<std::string, std::string>>
    match(http_method method, const std::string& target) const {
        std::unordered_map<std::string, std::string> params;
  ...
  private:
    ...
    void add_route(http_method method, std::string_view path, handler handle,
            ...
            if (segment[0] == ':') {
                ...
                if (!current->parameter_child) {
                    ...
                    throw std::runtime_error(
                        "Route conflict: different parameter names at same position: :"
                ...
                if (i != segments.size() - 1) {
                    throw std::runtime_error("Wildcard must be the last segment in route: "
                                             + path_str);
                }
                ...
                if (!current->wildcard_child) {
                    ...
                    throw std::runtime_error(
                        "Route conflict: different wildcard names at same position: *"
                ...
                if (current->static_children.find(segment) == current->static_children.end()) {
                    ...
                    // Build the full path for debugging/conflict detection
                    current->static_children[segment]->full_path =
                        current->full_path + "/" + segment;
                }
        ...
        if (!constraints.empty()) { current->constraints[method] = constraints; }
    }
     ...
     */
    bool match_node(RadixNode* node, const std::vector<std::string>& segments, size_t index,
                    http_method method, std::unordered_map<std::string, std::string>& params,
                    ...
                    if (constraints_it != node->constraints.end()) {
                        for (const auto& [param_name, constraint] : constraints_it->second) {
                            ...
                            if (param_it != params.end()) {
                                ...
                                else if (!constraint.pattern.empty()
                                         && !match_pattern(value, constraint.pattern)) {
        ...
        if (node->parameter_child) {
            ...
            if (match_node(node->parameter_child.get(), segments, index + 1, method, params,
                           result)) {
                return true;
            }
    ...
    void print_node(const RadixNode* node, int depth) const {
        ...
        if (node->parameter_child) { print_node(node->parameter_child.get(), depth + 1); }

        if (node->wildcard_child) { print_node(node->wildcard_child.get(), depth + 1); }
    }
};
} // namespace glz