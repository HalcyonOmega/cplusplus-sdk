...
struct http_connection_pool {
    ...
    std::mutex mtx;
    std::unordered_map<connection_key, std::vector<std::shared_ptr<asio::ip::tcp::socket>>,
                       connection_key_hash>
...
struct http_client {
    ...
    get(std::string_view url, const std::unordered_map<std::string, std::string>& headers = {}) {
    ...
    std::expected<response, std::error_code>
    post(std::string_view url, const std::string& body,
    ...
    template <class T>
    std::expected<response, std::error_code>
    post_json(std::string_view url, const T& data,
              const std::unordered_map<std::string, std::string>& headers = {}) {
        std::string json_str;
        auto ec = glz::write_json(data, json_str);
        if (ec) { return std::unexpected(std::make_error_code(std::errc::invalid_argument)); }
    ...
    std::shared_ptr<http_stream_connection> stream_request(const stream_request_params& params) {
        ...
        if (!url_result) {
            asio::post(*async_io_context, [on_error = params.on_error,
                                           error = url_result.error()]() { on_error(error); });
            return nullptr;
        }

        std::string method = params.method.empty() ? "GET" : params.method;

        return perform_stream_request(method, *url_result, params.body, params.headers,
                                      params.timeout, params.strategy, params.on_data,
                                      params.on_error, params.on_connect, params.on_disconnect);
    }

    // Asynchronous GET request
    template <typename CompletionHandler>
    void get_async(std::string_view url,
                   const std::unordered_map<std::string, std::string>& headers,
        ...
        perform_request_async("GET", *url_result, "", headers,
                              std::forward<CompletionHandler>(handler));
    }

    // Overload for get_async without completion handler (returns future)
    std::future<std::expected<response, std::error_code>>
    get_async(std::string_view url,
        ...
        get_async(url, headers,
                  [promise = std::move(promise)](
                      std::expected<response, std::error_code> result) mutable {
    ...
    std::future<std::expected<response, std::error_code>>
    post_async(std::string_view url, const std::string& body,
               const std::unordered_map<std::string, std::string>& headers = {}) {
        std::promise<std::expected<response, std::error_code>> promise;
        auto future = promise.get_future();
    ...
    template <class T>
    std::future<std::expected<response, std::error_code>>
    post_json_async(std::string_view url, const T& data,
                    const std::unordered_map<std::string, std::string>& headers = {}) {
        std::promise<std::expected<response, std::error_code>> promise;
        auto future = promise.get_future();
  ...
  private:
            ...
            worker_threads.emplace_back([this]() {
                while (running) {
                    try {
                        ...
                        if (async_io_context->stopped()) { async_io_context->restart(); }
    ...
    std::shared_ptr<http_stream_connection>
    perform_stream_request(const std::string& method, const url_parts& url, const std::string& body,
                           const std::unordered_map<std::string, std::string>& headers,
                           std::chrono::seconds timeout, stream_read_strategy strategy,
        ...
        auto internal_on_disconnect = [this, user_on_disconnect = std::move(on_disconnect),
                                       connection, url]() {
            connection->is_connected = false;
            // Call the user's handler if provided
            if (user_on_disconnect) { user_on_disconnect(); }
        ...
        connection->timer->async_wait(
            [connection, on_error, internal_on_disconnect](std::error_code ec) {
        ...
        if (connection->socket->is_open()) {
            ...
            resolver->async_resolve(
                ...
                [this, url, method, body, headers, connection, resolver,
                 on_data = std::move(on_data), on_error = std::move(on_error),
    ...
    void send_stream_request(const url_parts& url, const std::string& method,
        ...
        asio::async_write(
            *connection->socket, asio::buffer(*request_buffer),
    ...
    void read_stream_response(std::shared_ptr<http_stream_connection> connection,
                              http_data_handler on_data, http_error_handler on_error,
                              http_connect_handler on_connect,
                              http_disconnect_handler on_disconnect) {
            ...
            [this, connection, on_data = std::move(on_data), on_error = std::move(on_error),
             on_connect = std::move(on_connect), on_disconnect = std::move(on_disconnect)](
                ...
                std::string_view header_data{
                    asio::buffer_cast<const char*>(connection->buffer->data()), bytes_transferred};
                ...
                while (!header_data.starts_with("\r\n")) {
                    ...
                    if (colon_pos != std::string::npos) {
                        ...
                        std::string_view value = (value_start != std::string::npos)
                ...
                if (on_connect) { on_connect(response_headers); }

                if (parsed_status->status_code >= 400) {
                    // Status code indicates an error, so we report it and end the stream.
                    on_error(std::make_error_code(
                        std::errc::connection_refused)); // Generic error for now
                    if (on_disconnect) on_disconnect();
                    return;
                }
                ...
                if (it != response_headers.response_headers.end()) {
                    if (it->second.find("chunked") != std::string::npos) { is_chunked = true; }
                }
    ...
    void start_chunked_reading(std::shared_ptr<http_stream_connection> connection,
                               http_data_handler on_data, http_error_handler on_error,
                               http_disconnect_handler on_disconnect) {
        // Begin the chunk parsing loop.
        // The buffer may already contain the first chunk size line or more data.
        read_chunk_size(connection, std::move(on_data), std::move(on_error),
                        std::move(on_disconnect));
    }

    void read_chunk_size(std::shared_ptr<http_stream_connection> connection,
                         http_data_handler on_data, http_error_handler on_error,
                         http_disconnect_handler on_disconnect) {
        asio::async_read_until(
            *connection->socket, *connection->buffer, "\r\n",
            [this, connection, on_data = std::move(on_data), on_error = std::move(on_error),
             on_disconnect = std::move(on_disconnect)](std::error_code ec,
                ...
                std::string_view line_view{
                    asio::buffer_cast<const char*>(connection->buffer->data()),
                ...
                auto [ptr, parse_ec] = std::from_chars(
                    line_view.data(), line_view.data() + line_view.size(), chunk_size, 16);
    ...
    void read_chunk_body(std::shared_ptr<http_stream_connection> connection, size_t chunk_size,
        ...
        if (connection->buffer->size() >= total_to_read) {
            ...
            asio::post(connection->socket->get_executor(),
                       [this, connection, on_data = std::move(on_data),
        ...
        asio::async_read(
            *connection->socket, *connection->buffer,
            ...
            [this, connection, chunk_size, on_data = std::move(on_data),
             on_error = std::move(on_error),
             on_disconnect = std::move(on_disconnect)](std::error_code ec, std::size_t) mutable {
                if (ec || connection->should_stop) {
                    if (ec != asio::error::eof && ec != asio::error::operation_aborted
    ...
    void start_stream_reading(std::shared_ptr<http_stream_connection> connection,
                              http_data_handler on_data, http_error_handler on_error,
                              http_disconnect_handler on_disconnect) {
        // Dispatch to the appropriate strategy
        switch (connection->strategy) {
            case stream_read_strategy::bulk_transfer:
                start_stream_reading_bulk(connection, std::move(on_data), std::move(on_error),
                                          std::move(on_disconnect));
                break;
    ...
    void start_stream_reading_bulk(std::shared_ptr<http_stream_connection> connection,
                                   http_data_handler on_data, http_error_handler on_error,
                                   http_disconnect_handler on_disconnect) {
        ...
        asio::async_read(*connection->socket, *connection->buffer, asio::transfer_at_least(1),
                         [this, connection, on_data, on_error,
                          on_disconnect](std::error_code ec, std::size_t /*bytes_transferred*/) {
                             if (ec || connection->should_stop) {
                                 if (ec != asio::error::eof && ec != asio::error::operation_aborted
                                     && !connection->should_stop) {
                                     on_error(ec);
                             ...
                             start_stream_reading_bulk(connection, on_data, on_error,
                                                       on_disconnect);
                         });
    }

    void start_stream_reading_immediate(std::shared_ptr<http_stream_connection> connection,
                                        http_data_handler on_data, http_error_handler on_error,
                                        http_disconnect_handler on_disconnect) {
    ...
    std::expected<response, std::error_code>
    perform_sync_request(const std::string& method, const url_parts& url, const std::string& body,
                         const std::unordered_map<std::string, std::string>& headers) {
        ...
        try {
            ...
            std::string_view header_data{asio::buffer_cast<const char*>(response_buffer.data()),
                                         header_bytes};
            ...
            auto parsed_status = parse_http_status_line(status_line);
            if (!parsed_status) { return std::unexpected(parsed_status.error()); }
                ...
                if (colon_pos != std::string::npos) {
                    ...
                    if (name.length() == 14
                        ...
                        std::from_chars(value.data(), value.data() + value.size(), content_length);
                               ...
                               && strncasecmp(name.data(), "Connection", 10) == 0) {
            ...
            if (content_length > body_in_buffer) {
                ...
                asio::read(*socket, response_buffer, asio::transfer_exactly(remaining_to_read),
                           read_ec);
                if (read_ec) {
    ...
    template <typename CompletionHandler>
    void perform_request_async(const std::string& method, const url_parts& url,
                               const std::string& body,
                               const std::unordered_map<std::string, std::string>& headers,
                               CompletionHandler&& handler) {
        auto socket = connection_pool->get_connection(url.host, url.port);
        ...
        if (socket->is_open()) {
            send_request(socket, method, url, body, headers,
                         std::forward<CompletionHandler>(handler));
        } else {
            auto resolver = std::make_shared<asio::ip::tcp::resolver>(*async_io_context);
            resolver->async_resolve(
                url.host, std::to_string(url.port),
                [this, socket, resolver, method, url, body, headers,
                 handler = std::forward<CompletionHandler>(handler)](
                    std::error_code ec, asio::ip::tcp::resolver::results_type results) mutable {
                    if (ec) {
                    ...
                    connect_and_send(socket, results, method, url, body, headers,
                                     std::move(handler));
                });
        }
    }
    ...
    void connect_and_send(std::shared_ptr<asio::ip::tcp::socket> socket,
        ...
        asio::async_connect(*socket, results,
                            [this, socket, method, url, body, headers,
                             handler = std::forward<CompletionHandler>(handler)](
                                std::error_code ec, const asio::ip::tcp::endpoint&) mutable {
                                if (ec) {
                                    handler(std::unexpected(ec));
    ...
    template <typename CompletionHandler>
    void send_request(std::shared_ptr<asio::ip::tcp::socket> socket, const std::string& method,
                      const url_parts& url, const std::string& body,
                      const std::unordered_map<std::string, std::string>& headers,
                      CompletionHandler&& handler) {
        // Build HTTP request
        ...
        asio::async_write(*socket, asio::buffer(*request_str_ptr),
                          [this, socket, request_str_ptr, url,
                           handler = std::forward<CompletionHandler>(handler)](
    ...
    void parse_and_read_body(
        ...
        std::string_view header_section{asio::buffer_cast<const char*>(buffer->data()),
                                        header_size};
        ...
        header_section.remove_prefix(line_end
                                     + 2); // Move the view past the status line and its CRLF
        ...
        size_t remaining_to_read = (content_length > body_already_in_buffer)
                                       ? (content_length - body_already_in_buffer)
                                       : 0;

        asio::async_read(*socket, *buffer, asio::transfer_exactly(remaining_to_read),
                         [this, socket, buffer, url, status_code = parsed_status->status_code,
                          ...
                          handler = std::forward<CompletionHandler>(handler)](std::error_code ec,
                             ...
                             std::string body(asio::buffer_cast<const char*>(buffer->data()),
                                              buffer->size());
