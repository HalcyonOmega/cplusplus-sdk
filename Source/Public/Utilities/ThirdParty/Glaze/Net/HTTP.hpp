...
inline std::expected<HttpStatusLine, std::error_code>
parse_http_status_line(std::string_view status_line) {
    ...
    std::string_view version =
        status_line.substr(http_prefix.length(), first_space - http_prefix.length());

    std::string_view status_code_str =
        status_line.substr(first_space + 1, has_status_message ? second_space - first_space - 1
    ...
    if (has_status_message && second_space < status_line.length() - 1) {
        ...
        if (start == status_message.end()) {
            ...
            const auto end =
                std::find_if(status_message.rbegin(), status_message.rend(), [](unsigned char ch) {
    ...
    const bool valid_version = std::all_of(version.begin(), version.begin() + dot_pos,
                                           [](unsigned char ch) { return std::isdigit(ch); })
                               && std::all_of(version.begin() + dot_pos + 1, version.end(),
                                              [](unsigned char ch) { return std::isdigit(ch); });

    if (!valid_version) { return std::unexpected(std::make_error_code(std::errc::protocol_error)); }
    ...
    const auto [ptr, ec] = std::from_chars(
        status_code_str.data(), status_code_str.data() + status_code_str.size(), status_code);
