...
inline std::string join_strings(const std::vector<std::string>& vec, const std::string& delimiter) {
    ...
    for (size_t i = 1; i < vec.size(); ++i) { result += delimiter + vec[i]; }
    return result;
}
...
inline handler create_cors_middleware(const cors_config& config = {}) {
    return [config](const request& req, response& res) {
        ...
        if (origin_it != req.headers.end()) { origin = origin_it->second; }
        ...
        if (!origin.empty() && is_origin_allowed(config, origin)) {
            ...
            if (!config.exposed_headers.empty()) {
                res.header("Access-Control-Expose-Headers",
                           join_strings(config.exposed_headers, ", "));
            }
            ...
            if (is_preflight && config.handle_preflight) {
                ...
                res.header("Access-Control-Allow-Methods",
                           join_strings(config.allowed_methods, ", "));

                // Add allowed headers
                res.header("Access-Control-Allow-Headers",
                           join_strings(config.allowed_headers, ", "));
 ...
 */
inline handler restrictive_cors(const std::vector<std::string>& origins,
                                bool allow_credentials = false) {
    cors_config config;
