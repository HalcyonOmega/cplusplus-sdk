...
inline void sha1_process(sha1_context* context, const uint8_t data[64]) {
    ...
    for (int i = 0; i < 16; i++) {
        w[i] = (data[i * 4] << 24) | (data[i * 4 + 1] << 16) | (data[i * 4 + 2] << 8)
               | data[i * 4 + 3];
    }
...
inline void sha1_update(sha1_context* context, const uint8_t* data, size_t len) {
    ...
    if ((j + len) > 63) {
        ...
        for (; i + 63 < len; i += 64) { sha1_process(context, &data[i]); }
        j = 0;
    }
...
inline std::string generate_accept_key(std::string_view client_key) {
    ...
    fallback_sha1::sha1_update(&ctx, reinterpret_cast<const uint8_t*>(combined.data()),
                               combined.size());
...
struct websocket_server {
    using message_handler =
        std::function<void(std::shared_ptr<websocket_connection>, std::string_view, ws_opcode)>;
    using close_handler = std::function<void(std::shared_ptr<websocket_connection>)>;
    using error_handler =
        std::function<void(std::shared_ptr<websocket_connection>, std::error_code)>;
    ...
    inline void notify_open(std::shared_ptr<websocket_connection> conn, const request& req) {
        if (open_handler_) { open_handler_(conn, req); }
    }

    inline void notify_message(std::shared_ptr<websocket_connection> conn, std::string_view message,
                               ws_opcode opcode) {
        if (message_handler_) { message_handler_(conn, message, opcode); }
    }
    ...
    inline void notify_error(std::shared_ptr<websocket_connection> conn, std::error_code ec) {
        if (error_handler_) { error_handler_(conn, ec); }
    }

    inline bool validate_connection(const request& req) {
        if (validate_handler_) { return validate_handler_(req); }
        return true;
    }
  ...
  private:
    inline void perform_handshake(const request& req) {
        ...
        if (it == req.headers.end()
            || !std::equal(it->second.begin(), it->second.end(), websocket_str.begin(),
                           websocket_str.end(),
                           [](char a, char b) { return std::tolower(a) == std::tolower(b); })) {
            do_close();
            return;
        ...
        asio::async_write(
            ...
            [self, req, response_str = std::move(response_str)](std::error_code ec, std::size_t) {
                if (ec) {
                    if (self->server_) { self->server_->notify_error(self, ec); }
                    return;
                }
                ...
                if (self->server_) { self->server_->notify_open(self, req); }
    ...
    inline void on_read(std::error_code ec, std::size_t bytes_transferred) {
        if (ec) {
            if (server_) { server_->notify_error(shared_from_this(), ec); }
            return;
        }

        // Add received data to frame buffer
        frame_buffer_.insert(frame_buffer_.end(), read_buffer_.begin(),
        ...
        if (!is_closing_ && handshake_complete_) { start_read(); }
    }
    ...
    inline void process_frame(const uint8_t* data, std::size_t length) {
        ...
        while (offset < length) {
            ...
            if (length - offset < payload_length) { break; }
    ...
    inline void handle_frame(ws_opcode opcode, const uint8_t* payload, std::size_t length,
                             bool fin) {
        switch (opcode) {
            case ws_opcode::text:
            ...
            case ws_opcode::close: {
                ...
                if (length >= 2) {
                    ...
                    if (length > 2) {
                        reason =
                            std::string(reinterpret_cast<const char*>(payload + 2), length - 2);
                    }
                }

                if (!is_closing_) { send_close_frame(code, reason); }
            ...
            default: close(ws_close_code::protocol_error, "Unknown opcode"); break;
        }
    }
    ...
    inline void send_frame(ws_opcode opcode, std::string_view payload, bool fin = true) {
        ...
        asio::async_write(socket_, asio::buffer(frame), [self](std::error_code ec, std::size_t) {
            if (ec && self->server_) { self->server_->notify_error(self, ec); }
        });
    }
    ...
    inline void send_close_frame(ws_close_code code, std::string_view reason) {
        ...
        if (!reason.empty()) { std::copy(reason.begin(), reason.end(), payload.begin() + 2); }
    ...
    inline void write_frame_header(ws_opcode opcode, std::size_t payload_length, bool fin,
                                   uint8_t* header) {
    ...
    inline void do_close() {
        if (server_) { server_->notify_close(shared_from_this()); }
