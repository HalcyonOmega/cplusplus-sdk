...
struct ExtUnits final {
    std::optional<std::string_view>
        unitAscii{}; // ascii representation of the unit, e.g. "m^2" for square meters
    std::optional<std::string_view>
...
struct schema final {
    bool reflection_helper{}; // needed to support automatic reflection, because ref is a
                              // std::optional
    std::optional<std::string_view> ref{};
    using schema_number = std::optional<std::variant<int64_t, uint64_t, double>>;
    using schema_any =
        std::variant<std::monostate, bool, int64_t, uint64_t, double, std::string_view>;
    ...
    // std::optional<std::map<std::string_view, std::vector<std::string_view>>>
    // dependent_required{};
    ...
    std::optional<bool> ExtAdvanced{}; // flag to indicate that the parameter is advanced and can be
                                       // hidden in default views

    static constexpr auto schema_attributes{
        true}; // allowance flag to indicate metadata within glz::object(...)
...
template <> struct glz::meta<glz::detail::defined_formats> {
    ...
    static constexpr std::array keys{"date-time",    "date",          "time",
...
template <array_t T> struct to_json_schema<T> {
    template <auto Opts> static void op(auto& s, auto& defs) {
        ...
        if (!def.type) { to_json_schema<V>::template op<Opts>(def, defs); }
...
template <nullable_t T> struct to_json_schema<T> {
    template <auto Opts> static void op(auto& s, auto& defs) {
        ...
        auto it =
            std::find_if(type.begin(), type.end(), [&](const auto& str) { return str == "null"; });
        if (it == type.end()) { type.emplace_back("null"); }
    }
};
...
template <is_variant T> struct to_json_schema<T> {
    template <auto Opts> static void op(auto& s, auto& defs) {
        ...
        if constexpr (type_counts::n_number) { (*s.type).emplace_back("number"); }
        if constexpr (type_counts::n_string) { (*s.type).emplace_back("string"); }
        if constexpr (type_counts::n_bool) { (*s.type).emplace_back("boolean"); }
        if constexpr (type_counts::n_object) { (*s.type).emplace_back("object"); }
        if constexpr (type_counts::n_array) { (*s.type).emplace_back("array"); }
        if constexpr (type_counts::n_null) { (*s.type).emplace_back("null"); }
        s.oneOf = std::vector<schematic>(N);
        ...
        for_each<N>([&]<auto I>() {
            ...
            if (not schema_val.attributes.title) { schema_val.attributes.title = ids[I]; }
...
template <class T> consteval bool json_schema_matches_object_keys() {
    if constexpr (json_schema_t<T> && (count_members<json_schema_type<T>> > 0)) {
        ...
        for (const auto& key : json_schema_names) {
            if (!std::binary_search(fields.begin(), fields.end(), key)) { return false; }
        }
        return true;
    } else {
        return true;
...
template <const std::string_view& ref> auto consteval validate_ref() noexcept -> void {
    ...
    static_assert(!has_slash(ref), join_v<chars<"Slash in name: \"">, ref,
                                          chars<"\" in json schema references is not allowed">>);
#else
...
struct to_json_schema<T> {
    template <auto Opts> static void op(auto& s, auto& defs) {
        ...
        if constexpr (requires { meta<V>::required; }) {
            if (!s.required) { s.required = std::vector<sv>{}; }
            for (auto& v : meta<V>::required) { s.required->emplace_back(v); }
        }

        if constexpr (requires { meta<V>::examples; }) {
            if (!s.examples) { s.examples = std::vector<sv>{}; }
            for (auto& v : meta<V>::examples) { s.examples->emplace_back(v); }
        }
        ...
        for_each<N>([&]<auto I>() {
            ...
            if constexpr (N > 0 && json_schema_size > 0) {
                ...
                // but this struct may have fewer keys that don't match the full set of keys in the
                // struct T we therefore can't use `decode_hash_with_size` for either structure.
                ...
                if constexpr (schema_index < json_schema_size) {
                    // Experimented with a to_array approach, but the compilation times were
                    // significantly higher even when converting this access to a run-time access
                    // Tested with both creating a std::array and a heap allocated C-style array and
                    // storing in a unique_ptr
            ...
            if (!def.type) { to_json_schema<val_t>::template op<Opts>(def, defs); }
