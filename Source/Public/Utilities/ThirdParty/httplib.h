    ...
    bool enqueue(std::function<void()> fn) override {
        {
            ...
            if (max_queued_requests_ > 0 && jobs_.size() >= max_queued_requests_) { return false; }
...
inline std::wstring u8string_to_wstring(const char* s) {
    ...
    if (wlen > 0) {
        ...
        wlen = ::MultiByteToWideChar(
            CP_UTF8, 0, s, len, const_cast<LPWSTR>(reinterpret_cast<LPCWSTR>(ws.data())), wlen);
  ...
  private:
    ...
    bool start_with_case_ignore(const std::string& a, const char* b) const {
        ...
        for (size_t i = 0; i < b_len; i++) {
            if (case_ignore::to_lower(a[i]) != case_ignore::to_lower(b[i])) { return false; }
        }
        return true;
    }
...
inline std::string random_string(size_t length) {
    ...
    std::string result;
    for (size_t i = 0; i < length; i++) { result += data[engine() % (sizeof(data) - 1)]; }
    return result;
}
...
inline bool range_error(Request& req, Response& res) {
    if (!req.ranges.empty() && 200 <= res.Status && res.Status < 300) {
        ...
        for (auto& r : req.ranges) {
            ...
            if (last_pos == -1 || last_pos >= content_len) { last_pos = content_len - 1; }
...
bool process_multipart_ranges_data(const Request& req, const std::string& boundary,
    ...
    for (size_t i = 0; i < req.ranges.size(); i++) {
        ...
        if (!content(offset_and_length.first, offset_and_length.second)) { return false; }
        ctoken("\r\n");
    }
    ...
    for (const auto& entry : base_dirs_) {
        ...
        if (!req.path.compare(0, entry.mount_point.size(), entry.mount_point)) {
            ...
            if (detail::is_valid_path(sub_path)) {
                ...
                if (stat.is_file()) {
                    for (const auto& kv : entry.headers) { res.set_header(kv.first, kv.second); }
...
inline socket_t Server::create_server_socket(const std::string& host, int port, int socket_flags,
    ...
    return detail::create_socket(
        ...
        std::move(socket_options), [](socket_t sock, struct addrinfo& ai, bool& /*quit*/) -> bool {
            if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) { return false; }
    ...
    if (req.get_header_value("Expect") == "100-continue") {
        ...
        switch (status) {
            ...
            case StatusCode::ExpectationFailed_417:
                ...
                strm.write("\r\n");
                break;
            default: connection_closed = true; return write_response(strm, true, req, res);
        }
    }
...
ClientImpl::send_with_content_provider(const std::string& method, const std::string& path,
    ...
    if (max_timeout_msec_ > 0) { req.start_time_ = std::chrono::steady_clock::now(); }
...
inline void
ClientImpl::set_server_certificate_verifier(std::function<SSLVerifierResponse(SSL* ssl)> verifier) {
...
inline void SSLServer::update_certs(X509* cert, EVP_PKEY* private_key,
    ...
    if (client_ca_cert_store != nullptr) { SSL_CTX_set_cert_store(ctx_, client_ca_cert_store); }
}
...
SSLClient::connect_with_proxy(Socket& socket,
    ...
    if (!detail::process_client_socket(
            ...
            write_timeout_usec_, max_timeout_msec_, start_time, [&](Stream& strm) {
                ...
                if (max_timeout_msec_ > 0) { req2.start_time_ = std::chrono::steady_clock::now(); }
