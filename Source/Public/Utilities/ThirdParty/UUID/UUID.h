...
template <class ElementType, std::size_t Extent> using span = std::span<ElementType, Extent>;
#else
template <class ElementType, std::ptrdiff_t Extent> using span = gsl::span<ElementType, Extent>;
#endif
...
template <typename TChar> [[nodiscard]] constexpr inline bool is_hex(TChar const ch) noexcept {
...
template <typename CharT> inline constexpr CharT guid_encoder[17] = "0123456789abcdef";

template <> inline constexpr wchar_t guid_encoder<wchar_t>[17] = L"0123456789abcdef";
} // namespace detail
...
enum class uuid_variant {
    // NCS backward compatibility (with the obsolete Apollo Network Computing System 1.5 UUID
    // format) N bit pattern: 0xxx > the first 6 octets of the UUID are a 48-bit timestamp (the
    // number of 4 microsecond units of time since 1 Jan 1980 UTC); > the next 2 octets are
    // reserved; > the next octet is the "address family"; > the final 7 octets are a 56-bit host ID
    // in the form specified by the address family
    ncs,
...
// https://en.wikipedia.org/wiki/X.500, https://en.wikipedia.org/wiki/Abstract_Syntax_Notation_One)
...
class uuid_system_generator {
  public:
    ...
    uuid operator()() {
        ...
        std::array<uint8_t, 16> bytes = {{id[0], id[1], id[2], id[3], id[4], id[5], id[6], id[7],
...
template <typename UniformRandomNumberGenerator> class basic_uuid_random_generator {
  public:
...
class uuid_name_generator {
  public:
    ...
    template <typename StringType> [[nodiscard]] uuid operator()(StringType const& name) {
        reset();
...
template <> struct hash<uuids::uuid> {
    ...
    [[nodiscard]] result_type operator()(argument_type const& uuid) const {
        ...
        uint64_t h =
            ...
            | static_cast<uint64_t>(uuid.data[13]) << 16 | static_cast<uint64_t>(uuid.data[14]) << 8
