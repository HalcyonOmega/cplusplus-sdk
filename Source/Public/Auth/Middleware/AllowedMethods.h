...
/**
 * Middleware to handle unsupported HTTP methods with a HTTPStatus::MethodNotAllowed Method Not
 * Allowed response.
 *
 * @param InAllowedMethods Array of allowed HTTP methods for this endpoint (e.g., {MTHD_GET,
 * MTHD_POST})
 * @returns Express middleware that returns a HTTPStatus::MethodNotAllowed error if method not in
 * allowed list
 */
