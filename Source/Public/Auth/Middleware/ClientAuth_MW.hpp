...
struct HTTPResponse {
    ...
    void SetStatus(int Status) { StatusCode = Status; }
    void SetJSON(const JSON& Data) { Body = Data; }
};
...
RequestHandler AuthenticateClient(const ClientAuthenticationMiddlewareOptions& Options) {
    return [Options](HTTPRequest& Request, HTTPResponse& Response, NextFunction Next) -> future<void> {
        ...
        try {
            ...
            OnClientRetrievedCallback OnClientRetrieved = [Promise, ClientSecret, &Request, &Response, Next]
            (shared_ptr<OAuthClientInformationFull> Client) {
                try {
                    if (!Client) {
...
struct AuthenticationTask {
    struct promise_type {
        ...
        void return_value(bool Value) { Result = Value; }
        void unhandled_exception() { Result = false; }
    };

    coroutine_handle<promise_type> Handle;

    explicit AuthenticationTask(coroutine_handle<promise_type> H) : Handle(H) {}
    ~AuthenticationTask() { if (Handle) Handle.destroy(); }

    bool GetResult() const { return Handle.promise().Result; }
