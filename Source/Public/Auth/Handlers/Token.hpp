...
class OAuthError : public exception {
  public:
    ...
    OAuthError(const string& code, const string& desc, const string& type = "OAuthError")
        : ErrorCode(code), Description(desc), ErrorType(type) {}
...
struct Client {
    string ClientId;
    string ClientSecret;
    vector<string> AllowedGrantTypes;
