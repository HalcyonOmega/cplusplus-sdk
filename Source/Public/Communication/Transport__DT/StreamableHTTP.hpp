...
class EventStore {
  public:
     ...
     */
    virtual future<EventID> storeEvent(const StreamID& StreamID,
                                       const JSON_RPC_Message& message) = 0;
        ...
        return async(launch::async, [this, message, relatedRequestID]() {
            ...
            if (isResponse) {
                ...
                if (message.data.contains(MSG_KEY_ID)) {
                    RequestID = message.data[MSG_KEY_ID];
                }
            }
  ...
  private:
    ...
    void scheduleReconnection(const StartSSEOptions& options, int attemptCount = 0) {
        ...
        if (maxRetries > 0 && attemptCount >= maxRetries) {
            if (onerror) {
                string msg =
                    "Maximum reconnection attempts (" + to_string(maxRetries) + ") exceeded.";
