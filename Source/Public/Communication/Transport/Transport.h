...
class EventStore {
  public:
    ...
    virtual std::string StoreEvent(const std::string& streamId, const MessageBase& message) = 0;
    virtual std::string
    ReplayEventsAfter(const std::string& lastEventId,
                      std::function<void(const std::string&, const MessageBase&)> send) = 0;
};
...
class Transport {
  public:
    ...
    virtual std::future<void> Send(const MessageBase& message,
                                   const TransportSendOptions& options = {}) = 0;
    ...
    using MessageCallback = std::function<void(const MessageBase&, const AuthInfo*)>;
    ...
    virtual bool WriteSSEEvent(const MessageBase& message, const std::string& eventId = "") {
        (void)message;
        (void)eventId;
        return false;
    }
  ...
  protected:
    ...
    std::map<StreamId, std::shared_ptr<MessageBase>> streamMapping;
    std::map<RequestID, StreamId> requestToStreamMapping;
    std::map<RequestID, std::shared_ptr<MessageBase>> requestResponseMap;
};
