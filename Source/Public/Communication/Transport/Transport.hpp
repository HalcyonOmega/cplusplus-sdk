...
public:
  ...
  // Sends a JSON-RPC message (request or response). If present, `relatedRequestID` is used to indicate to the transport which incoming request to associate this outgoing message with.
  virtual future<void> Send(const JSON_RPC_Message& Message, const optional<TransportSendOptions>& Options = nullopt) = 0;
  ...
  // Callback for when an error occurs. Note that errors are not necessarily fatal; they are used for reporting any kind of exceptional condition out of band.
  optional<function<void(const exception&)>> OnError;

  // Callback for when a message (request or response) is received over the connection. Includes the authInfo if the transport is authenticated.
