 ...
 */
template<typename RequestT = Request, typename NotificationT = Notification, typename ResultT = Result>
...
public:
    async<void> Connect(shared_ptr<Transport> TransportPtr, const optional<RequestOptions>& Options = nullopt) override;
    ...
    async<JSON> SetLoggingLevel(LoggingLevel Level, const optional<RequestOptions>& Options = nullopt);

    async<JSON> GetPrompt(const JSON& Params, const optional<RequestOptions>& Options = nullopt);

    async<JSON> ListPrompts(const optional<JSON>& Params = nullopt, const optional<RequestOptions>& Options = nullopt);

    async<JSON> ListResources(const optional<JSON>& Params = nullopt, const optional<RequestOptions>& Options = nullopt);

    async<JSON> ListResourceTemplates(const optional<JSON>& Params = nullopt, const optional<RequestOptions>& Options = nullopt);

    async<JSON> ReadResource(const JSON& Params, const optional<RequestOptions>& Options = nullopt);

    async<JSON> SubscribeResource(const JSON& Params, const optional<RequestOptions>& Options = nullopt);

    async<JSON> UnsubscribeResource(const JSON& Params, const optional<RequestOptions>& Options = nullopt);

    async<JSON> CallTool(const JSON& Params, const string& ResultSchema = "CallToolResultSchema", const optional<RequestOptions>& Options = nullopt);

    async<JSON> ListTools(const optional<JSON>& Params = nullopt, const optional<RequestOptions>& Options = nullopt);
...
Client<RequestT, NotificationT, ResultT>::Client(const Implementation& ClientInfo, const optional<ClientOptions>& Options)
    : Protocol<ClientRequest, ClientNotification, ClientResult>(Options)
    , ClientInfo_(ClientInfo)
    , Capabilities_(Options ? Options->Capabilities.value_or(ClientCapabilities{}) : ClientCapabilities{})
    , Ajv_()
{
}

template<typename RequestT, typename NotificationT, typename ResultT>
void Client<RequestT, NotificationT, ResultT>::RegisterCapabilities(const ClientCapabilities& Capabilities) {
    if (GetTransport()) {
        throw runtime_error("Cannot register capabilities after connecting to transport");
    }

    // Exactly matching TypeScript: this._capabilities = mergeCapabilities(this._capabilities, capabilities);
    // TODO: Implement MergeCapabilities function
    // Capabilities_ = MergeCapabilities(Capabilities_, Capabilities);
}

template<typename RequestT, typename NotificationT, typename ResultT>
void Client<RequestT, NotificationT, ResultT>::AssertCapability(const string& Capability, const string& Method) {
...
template<typename RequestT, typename NotificationT, typename ResultT>
async<void> Client<RequestT, NotificationT, ResultT>::Connect(shared_ptr<Transport> TransportPtr, const optional<RequestOptions>& Options) {
    ...
    try {
        JSON InitializeRequest = JSON{
            {"method", "initialize"},
            {"params", JSON{
                {"protocolVersion", "LATEST_PROTOCOL_VERSION"}, // TODO: Use actual LATEST_PROTOCOL_VERSION constant
                {"capabilities", JSON::object()}, // TODO: Serialize Capabilities_ properly
                {"clientInfo", JSON::object()} // TODO: Serialize ClientInfo_ properly
            }}
        };
        ...
        //     throw runtime_error("Server's protocol version is not supported: " + Result["protocolVersion"].get<string>());
        // }

        // TODO: Parse and set ServerCapabilities_, ServerVersion_, Instructions_ from Result
        // ServerCapabilities_ = ParseServerCapabilities(Result["capabilities"]);
        // ServerVersion_ = ParseImplementation(Result["serverInfo"]);
        // if (Result.contains("instructions")) {
        //     Instructions_ = Result["instructions"].get<string>();
        // }

        JSON InitializedNotification = JSON{{"method", "notifications/initialized"}};
...
template<typename RequestT, typename NotificationT, typename ResultT>
optional<ServerCapabilities> Client<RequestT, NotificationT, ResultT>::GetServerCapabilities() const {
...
void Client<RequestT, NotificationT, ResultT>::AssertCapabilityForMethod(const string& Method) {
    if (Method == "logging/setLevel") {
        if (!ServerCapabilities_ || !ServerCapabilities_->Logging) { // TODO: Access logging capability properly
            throw runtime_error("Server does not support logging (required for " + Method + ")");
        }
    } else if (Method == "prompts/get" || Method == "prompts/list") {
        if (!ServerCapabilities_ || !ServerCapabilities_->Prompts) { // TODO: Access prompts capability properly
        ...
        if (!ServerCapabilities_ || !ServerCapabilities_->Resources) { // TODO: Access resources capability properly
        ...
        if (Method == "resources/subscribe") {
            ...
            //     throw runtime_error("Server does not support resource subscriptions (required for " + Method + ")");
            // }
        }
    } else if (Method == "tools/call" || Method == "tools/list") {
        if (!ServerCapabilities_ || !ServerCapabilities_->Tools) { // TODO: Access tools capability properly
            throw runtime_error("Server does not support tools (required for " + Method + ")");
        }
    } else if (Method == "completion/complete") {
        if (!ServerCapabilities_ || !ServerCapabilities_->Completions) { // TODO: Access completions capability properly
            throw runtime_error("Server does not support completions (required for " + Method + ")");
        }
    }
    // "initialize" and "ping" require no specific capability
}
...
void Client<RequestT, NotificationT, ResultT>::AssertNotificationCapability(const string& Method) {
    if (Method == "notifications/roots/list_changed") {
        ...
        //     throw runtime_error("Client does not support roots list changed notifications (required for " + Method + ")");
        // }
    }
    // "notifications/initialized", "notifications/cancelled", "notifications/progress" require no specific capability
}

template<typename RequestT, typename NotificationT, typename ResultT>
void Client<RequestT, NotificationT, ResultT>::AssertRequestHandlerCapability(const string& Method) {
    if (Method == "sampling/createMessage") {
        // TODO: Check Capabilities_.Sampling properly
        // if (!Capabilities_.Sampling) {
        //     throw runtime_error("Client does not support sampling capability (required for " + Method + ")");
        // }
    } else if (Method == "roots/list") {
        // TODO: Check Capabilities_.Roots properly
        // if (!Capabilities_.Roots) {
        //     throw runtime_error("Client does not support roots capability (required for " + Method + ")");
        // }
    }
    // "ping" requires no specific capability
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::Ping(const optional<RequestOptions>& Options) {
    return co_await Request(JSON{{"method", "ping"}}, "EmptyResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::Complete(const JSON& Params, const optional<RequestOptions>& Options) {
    JSON CompleteRequest = JSON{{"method", "completion/complete"}, {"params", Params}};
    return co_await Request(CompleteRequest, "CompleteResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::SetLoggingLevel(LoggingLevel Level, const optional<RequestOptions>& Options) {
    JSON SetLevelRequest = JSON{{"method", "logging/setLevel"}, {"params", JSON{{"level", static_cast<int>(Level)}}}};
    return co_await Request(SetLevelRequest, "EmptyResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::GetPrompt(const JSON& Params, const optional<RequestOptions>& Options) {
    JSON GetPromptRequest = JSON{{"method", "prompts/get"}, {"params", Params}};
    return co_await Request(GetPromptRequest, "GetPromptResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::ListPrompts(const optional<JSON>& Params, const optional<RequestOptions>& Options) {
    JSON ListPromptsRequest = JSON{{"method", "prompts/list"}};
    if (Params) {
...
template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::ListResources(const optional<JSON>& Params, const optional<RequestOptions>& Options) {
    JSON ListResourcesRequest = JSON{{"method", "resources/list"}};
    if (Params) {
...
template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::ListResourceTemplates(const optional<JSON>& Params, const optional<RequestOptions>& Options) {
    JSON ListResourceTemplatesRequest = JSON{{"method", "resources/templates/list"}};
    if (Params) {
        ListResourceTemplatesRequest["params"] = *Params;
    }
    return co_await Request(ListResourceTemplatesRequest, "ListResourceTemplatesResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::ReadResource(const JSON& Params, const optional<RequestOptions>& Options) {
    JSON ReadResourceRequest = JSON{{"method", "resources/read"}, {"params", Params}};
    return co_await Request(ReadResourceRequest, "ReadResourceResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::SubscribeResource(const JSON& Params, const optional<RequestOptions>& Options) {
    JSON SubscribeRequest = JSON{{"method", "resources/subscribe"}, {"params", Params}};
    return co_await Request(SubscribeRequest, "EmptyResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::UnsubscribeResource(const JSON& Params, const optional<RequestOptions>& Options) {
    JSON UnsubscribeRequest = JSON{{"method", "resources/unsubscribe"}, {"params", Params}};
    return co_await Request(UnsubscribeRequest, "EmptyResultSchema", Options);
}

template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::CallTool(const JSON& Params, const string& ResultSchema, const optional<RequestOptions>& Options) {
    JSON CallToolRequest = JSON{{"method", "tools/call"}, {"params", Params}};
    ...
    if (Validator) {
        ...
        if (!Result.contains("structuredContent") && !Result.value("isError", false)) {
            ...
            try {
                ...
                if (!IsValid) {
                    throw MCP_Error(ErrorCode::InvalidParams,
                        "Structured content does not match the tool's output schema: " + Ajv_.ErrorsText(Ajv_.Errors));
                }
...
void Client<RequestT, NotificationT, ResultT>::CacheToolOutputSchemas(const vector<Tool>& Tools) {
    ...
    for (const auto& ToolItem : Tools) {
        ...
        //     } catch (const exception& Error) {
        //         // Log warning: "Failed to compile output schema for tool " + ToolItem.Name + ": " + Error.what()
        //     }
        // }
    }
}
...
template<typename RequestT, typename NotificationT, typename ResultT>
optional<ValidateFunction> Client<RequestT, NotificationT, ResultT>::GetToolOutputValidator(const string& ToolName) {
...
template<typename RequestT, typename NotificationT, typename ResultT>
async<JSON> Client<RequestT, NotificationT, ResultT>::ListTools(const optional<JSON>& Params, const optional<RequestOptions>& Options) {
    JSON ListToolsRequest = JSON{{"method", "tools/list"}};
    if (Params) {
...
async<void> Client<RequestT, NotificationT, ResultT>::SendRootsListChanged() {
    JSON RootsListChangedNotification = JSON{{"method", "notifications/roots/list_changed"}};
