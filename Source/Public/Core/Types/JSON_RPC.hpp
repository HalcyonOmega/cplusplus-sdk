...
bool IsJSON_RPC_Notification(const JSON& value) {
  return value.is_object() &&
         value.value("jsonrpc", "") == JSON_RPC_VERSION &&
...
struct JSON_RPC_Error {
  ...
  struct {
    ...
    string Message; // A short description of the error. The message SHOULD be limited to a concise single sentence.
    optional<any> Data; // Additional information about the error. The value of this member is defined by the sender (e.g. detailed error information, nested errors etc.).
  } Error;
};
...
using JSON_RPC_Message = variant<
  JSON_RPC_Request,
