...
struct CancelledNotification : public Notification {
  ...
  struct {
    ...
    optional<string> Reason; // An optional string describing the reason for the cancellation. This MAY be logged or presented to the user.
  } Params;
...
const isInitializedNotification = (value: unknown): value is InitializedNotification =>
  InitializedNotification.safeParse(value).success;

/* Progress notifications */
struct Progress {
  double Progress; // The progress thus far. This should increase every time progress is made, even if the total is unknown.
  optional<double> Total; // Total number of items to process (or total progress required), if known.
...
struct ResourceUpdatedNotification : public Notification {
  struct {
    BaseNotificationParams;
    string URI; // The URI of the resource that has been updated. This might be a sub-resource of the one that the client actually subscribed to.
  } Params;
...
struct LoggingMessageNotification : public Notification {
  struct {
    ...
    any Data; // The data to be logged, such as a string message or an object. Any JSON serializable type is allowed here.
  } Params;
