...
MCPProtocol::MCPProtocol(std::unique_ptr<ITransport> InTransport)
    : m_Transport(std::move(InTransport)), m_MessageManager(std::make_unique<MessageManager>()), m_State() {
...
MCPTask<PingResponse> MCPProtocol::Ping(const PingRequest& InRequest) { co_return PingResponse{InRequest.GetRequestID()}; }
...
MCPTask_Void MCPProtocol::SendRequest(const RequestBase& InRequest) {
    ...
    std::string RequestID = InRequest.ID.ToString();
...
MCPTask_Void MCPProtocol::SendErrorResponse(const ErrorResponseBase& InErrorResponse) const
{
