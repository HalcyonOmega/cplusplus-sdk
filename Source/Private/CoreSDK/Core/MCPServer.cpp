...
MCPServer::MCPServer(TransportType InTransportType,
    ...
    : MCPProtocol(TransportFactory::CreateTransport(
          InTransportType, TransportSide::Server, std::move(InOptions))) {
...
void MCPServer::OnRequest_Initialize(const InitializeRequest &InRequest) {
  try {
    ...
    auto iter =
        std::find(SUPPORTED_PROTOCOL_VERSIONS.begin(),
...
bool MCPServer::AddTool(const Tool &InTool,
                        const ToolManager::ToolFunction &InFunction) {
...
bool MCPServer::AddPrompt(const Prompt &InPrompt,
                          const PromptManager::PromptFunction &InFunction) {
...
bool MCPServer::RemoveResource(const Resource &InResource) {
  if (!m_ResourceManager->RemoveResource(InResource)) {
    HandleRuntimeError("Failed to remove resource: " +
                       InResource.URI.toString());
    return false;
  }
...
bool MCPServer::RemoveResourceTemplate(const ResourceTemplate &InTemplate) {
  if (!m_ResourceManager->RemoveTemplate(InTemplate)) {
    HandleRuntimeError("Failed to remove resource template: " +
                       InTemplate.Name);
    return false;
  }
...
MCPTask_Void MCPServer::Notify_ResourceUpdated(
    const ResourceUpdatedNotification::Params &InParams) {
...
void MCPServer::OnRequest_UnsubscribeResource(
    const UnsubscribeRequest &InRequest) {
  try {
...
MCPTask_Void MCPServer::Notify_LogMessage(
...
MCPServer::Request_CreateMessage(const CreateMessageRequest::Params &InParams) {
  CreateMessageResponse Response =
      co_await SendRequest(CreateMessageRequest(InParams));
...
void MCPServer::OnRequest_Complete(const CompleteRequest &InRequest) {
  try {
    ...
    if (!m_CompletionHandler) {
      SendErrorResponse(ErrorResponseBase(
          InRequest.GetRequestID(),
          MCPError(ErrorCodes::METHOD_NOT_FOUND, "Completion not supported")));
      return;
    }
...
MCPTask_Void
MCPServer::Notify_Progress(const ProgressNotification::Params &InParams) {
  co_await SendNotification(ProgressNotification(InParams));
}

MCPTask_Void
MCPServer::Notify_CancelRequest(const CancelledNotification::Params &InParams) {
  co_await SendNotification(CancelledNotification(InParams));
}

void MCPServer::OnNotified_Progress(
    const ProgressNotification &InNotification) {
...
void MCPServer::OnNotified_CancelRequest(
    const CancelledNotification &InNotification) {
  // Ensure this method requires instance access
  if (!IsInitialized()) {
    return;
  }
...
void MCPServer::SetupDefaultHandlers() {
  ...
  m_MessageManager->RegisterRequestHandler<SubscribeRequest>(
      [this](const auto &request) {
        return OnRequest_SubscribeResource(request);
      });
  m_MessageManager->RegisterRequestHandler<UnsubscribeRequest>(
      [this](const auto &request) {
        return OnRequest_UnsubscribeResource(request);
      });
...
MCPTask_Void MCPServer::UpdateProgress(double InProgress,
