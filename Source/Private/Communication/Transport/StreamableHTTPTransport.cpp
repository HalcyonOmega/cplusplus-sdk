...
StreamableHTTPTransport::StreamableHTTPTransport(const string& InURL)
...
future<void> StreamableHTTPTransport::Start() {
...
future<void> StreamableHTTPTransport::Close() {
    if (!_isRunning) { return; }
...
future<void> StreamableHTTPTransport::Send(const MessageBase& InMessage,
...
void StreamableHTTPTransport::WriteSSEEvent(const string& InEvent, const string& InData) {
...
bool StreamableHTTPTransport::Resume(const string& InResumptionToken) {
...
void StreamableHTTPTransport::ParseSSEData(const string& data) {
    size_t pos = 0;
    string currentEvent;
    string currentData;
...
optional<string> StreamableHTTPClientTransport::getSessionID() const {
    return session_id_;
}
