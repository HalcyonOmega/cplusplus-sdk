...
StreamableHTTPError(optional<int> code, const string& message) : code_(code), message_(message) {
    full_message_ = "Streamable HTTP error: " + message;
}

const char* StreamableHTTPError::what() const noexcept {
    return full_message_.c_str();
}

optional<int> StreamableHTTPError::getCode() const {
    return code_;
}

const string& StreamableHTTPError::getMessage() const {
    return message_;
}
