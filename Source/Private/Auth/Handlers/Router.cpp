...
void URLHelper::ParseURL(const string& URLString) {
    Href = URLString;
...
void ExpressRouter::Use(const string& Path, RequestHandler Handler) {
    Routes[Path] = Handler;
}

void ExpressRouter::Use(RequestHandler Handler) {
    Middleware.push_back(Handler);
}

RequestHandler ExpressRouter::CreateHandler() {
