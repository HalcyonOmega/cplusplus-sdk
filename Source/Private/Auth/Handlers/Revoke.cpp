...
// TODO: Fix External Ref: HTTP::Request/HTTP::Response
...
RequestHandler RevocationHandler(const RevocationHandlerOptions& options) {
    ...
    return [options](shared_ptr<HTTP::Request> req,
                     shared_ptr<HTTP::Response> res) -> AsyncRequestHandler {
        try {
            ...
            int status = dynamic_cast<const ServerError*>(&error) ? 500 : HTTP::Status::BadRequest;
